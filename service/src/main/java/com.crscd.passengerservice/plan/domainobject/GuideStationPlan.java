 ...
 * 导向计划属性与客运计划属性相同
 */
public class GuideStationPlan extends BaseTrainPlan {
    /**************** 检票信息 *****************/
    /* 进站上车开检时间基准 */
    ...
    public GuideStationPlan(BasicPlan plan, String date, String stationName, OrganizeTemplate template, ConfigManager manager) {
    ...
    // 通过po读取计划
    public GuideStationPlan(GuideStationPlanBean bean, ConfigManager manager) {
        super(manager);
        this.id = bean.getId();
    ...
    public LocalDateTime getAutoStopAboardCheckTime() {
        if (!this.getStationType().equals(StationTypeEnum.FINAL)) {
            return calcActualCheckTime(this.getStopAboardCheckBase(), this.getStopAboardCheckTimeOffset());
        }
        return null;
    }
