 ...
 * Time: 15:35
 */
public class GuideStationPlanManager extends AbstractPlanManager<GuideStationPlan, GuidePlanDataPool> {
    public GuideStationPlanManager() {
    }

    public boolean updatePlan(String planKey, Map<GuidePlanModifyEnum, String> modifyList, LateEarlyReasonEnum modifyStartReason, LateEarlyReasonEnum modifyDepartureReason) {
        GuideStationPlan plan = getPlanDataPool().getPlan(planKey);
        for (Map.Entry<GuidePlanModifyEnum, String> entry : modifyList.entrySet()) {
            switch (entry.getKey()) {
                ...
                case ACTUAL_ARRIVE_TIME:
                    ...
                    if (null == modifyStartReason) {
                        plan.setArriveStateReason(LateEarlyReasonEnum.NONE);
                    } else {
                        plan.setArriveStateReason(modifyStartReason);
                    }
                    break;
    ...
    public boolean modifyPlan(NoticeMessage notice) {
        if (!notice.getReceiver().equals(ReceiverEnum.GUIDE_PLAN)) {
